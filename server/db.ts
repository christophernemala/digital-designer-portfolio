...
export async function createPortfolioProject(
  userId: number,
  data: Omit<InsertPortfolioProject, 'userId'>
) {
  const db = await getDb();
  if (!db) throw new Error('Database not available');
  
  return db.insert(portfolioProjects).values({
    ...data,
    userId,
  });
}
...
/**
 * Portfolio Files Queries
 */
export async function uploadPortfolioFile(
  userId: number,
  projectId: number,
  fileData: Omit<InsertPortfolioFile, 'userId' | 'projectId'>
) {
  const db = await getDb();
  if (!db) throw new Error('Database not available');
  
  return db.insert(portfolioFiles).values({
    ...fileData,
    userId,
    projectId,
  });
